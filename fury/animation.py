...
vertex_shader_code_impl = \
    ...
    vec4 a = vertexMC ; // + vec4(time, 0, 0, 0);
...
class Timeline:
    ...
    def update(self):
        if not self._use_shaders:
            ...
            color = np.clip(self.get_color(t), 0, 1) * 255
            for actor in self.get_actors():
                # actor.GetShaderProperty().GetVertexCustomUniforms().SetUniformf('time', t)
                # print(actor.GetShaderProperty().GetVertexCustomUniforms())
