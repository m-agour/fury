...
fn compute_spherical_harmonic(l: i32, m: i32, theta: f32, phi: f32) -> f32 {
    ...
    // Apply phase and trigonometric factors
...
fn compute_sh_gradient_magnitude(theta: f32, phi: f32) -> f32 {
    // Approximate gradient magnitude for normal estimation
    ...
    for (var l: i32 = 0; l <= L_MAX; l++) {
        for (var m: i32 = -l; m <= l; m++) {
            ...
            // Compute derivatives with respect to theta and phi
    ...
    return sqrt(gradient_sum) + 1e-6; // Add small epsilon to avoid division by zero
}
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // Precompute all SH basis functions for this direction
    var coeff_idx = 0;
    for (var l: i32 = 0; l <= L_MAX; l++) {
        for (var m: i32 = -l; m <= l; m++) {
            if (coeff_idx >= NUM_COEFFS) {
    ...
    // Precompute normal estimation factor
    ...
    // Precompute Legendre polynomials for common L,M pairs
    var legendre_idx = 0u;
    for (var l: i32 = 0; l <= L_MAX; l++) {
